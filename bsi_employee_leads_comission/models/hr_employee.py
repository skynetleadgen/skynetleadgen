...
class HrEmployeePrivate(models.Model):
    ...
    total_sales_by_employee = fields.Float(
        string="Total sales by employee", compute="compute_total"
    )
    opportunity_count = fields.Integer(string="Opportunities", compute="compute_count")
    total_commission = fields.Float(
        string="Total Commission", compute="compute_commission"
    )
    commission_rule_id = fields.Many2one(
        "crm.commission.rule", string="Commission Rule"
    )
    ...
    def compute_commission(self):
        for record in self:
            if record.commission_rule_id.commission_type == "percentage":
                percentage_count = (
                    record.total_sales_by_employee
                    * record.commission_rule_id.commission
                    / 100
                )
